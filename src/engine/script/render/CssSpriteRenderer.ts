...
export class CssSpriteRenderer extends Component {
    ...
    public override onDestroy(): void {
        ...
        if (this._sprite) this.gameObject.transform.unsafeGetObject3D().remove(this._sprite); //it's safe because _sprite is not GameObject and remove is from onDestroy
    }
    ...
    public asyncSetImagePath(path: string|null, onComplete?: () => void): void {
        ...
        const onLoad = (e: Event) => {
            ...
            if (!this._sprite) {
                ...
                this.gameObject.transform.unsafeGetObject3D().add(this._sprite); //it's safe because _sprite is not GameObject and remove is from onDestroy
